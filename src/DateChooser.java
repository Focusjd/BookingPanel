...
/** 
 * Date selector, you can specify the date display format
 */
public class DateChooser extends JPanel {
	...
	/** 
	 *Gets the date of the current selection box
	 */
	public Date getDate() {
	...
	//According to the initialization date, initialize the panel  
	private void initPanel() {
		...
		this.addAncestorListener(new AncestorListener() {
			...
			//As soon as the ancestor component moves, pop will disappear immediately  
	...
	/** 
	 * The top panel is used to show the increase or decrease of the month
	 */
	private class JP1 extends JPanel {
		...
		private void updateDate() {
			center.setText( + (select.get(Calendar.MONTH) + 1) + ", "+select.get(Calendar.YEAR) );
		}
	}
	...
	private class JP2 extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			g.setFont(font);
			g.drawString("Sun      Mon     Tue     Wed    Thu      Fri      Sat", 5, 10);
	...
	private class MyLabel extends JLabel implements Comparator<MyLabel>, MouseListener, MouseMotionListener {
		...
		protected void paintComponent(Graphics g) {
			if (day == select.get(Calendar.DAY_OF_MONTH) && month == select.get(Calendar.MONTH)) {
				//Highlight if the current date is a selection date 
				g.setColor(new Color(160, 185, 215));
				g.fillRect(0, 0, getWidth(), getHeight());
			}
			if (year == now.get(Calendar.YEAR) && month == now.get(Calendar.MONTH) && day == now.get(Calendar.DAY_OF_MONTH)) {
				//If the date is the same as the current date, use the red box  
			...
			if (isSelected) {//If selected, draw a dotted line box 
	...
	private class LabelManager {
		...
		public void setSelect(Point p, boolean b) {
			//If it's dragging, you need to optimize it to improve efficiency 
			if (b) {
				//It indicates whether it can return. You don't need to compare all the tags. 
				//The flag that can be returned is to compare the previous tag with the previous one  
				//Just find the label to be displayed  
	...
	private class JP4 extends JPanel {
		...
		public JP4() {
			...
			jl.setToolTipText("Click and choose date");
